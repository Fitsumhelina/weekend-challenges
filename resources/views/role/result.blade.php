                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100 flex flex-col md:table-row md:flex-row md:items-center w-full md:w-auto">
                    ...
                    <td class="py-3 px-6 text-center md:table-cell flex justify-between md:block">
                        ...
                        <div class="flex item-center justify-center space-x-2">
                            @can('update role')
                                <button class="edit-role-btn w-8 h-8 rounded-full bg-yellow-100 text-yellow-600 hover:bg-yellow-200 flex items-center justify-center transition duration-300" data-id="{{ $role->id }}" title="Edit">
                          ...
                          @can('delete role')
                            <form action="{{ route('role.destroy', $role->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="delete-role-btn w-8 h-8 rounded-full bg-red-100 text-red-600 hover:bg-red-200 flex items-center justify-center transition duration-300" title="Delete">
