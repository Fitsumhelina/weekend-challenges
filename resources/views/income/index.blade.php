...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white shadow-lg rounded-lg p-6 mb-8">
        <div class="flex justify-between items-center mb-6">
            ...
            @can('create income')
                <button id="createIncomeBtn" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out">
                    Add New Income
                </button>
            @endcan
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        viewIncomeModal.addEventListener('click', function(event) {
        ...
        if (createIncomeBtn) {
            createIncomeBtn.addEventListener('click', function () {
                modalTitle.textContent = 'Create New Income';
                incomeForm.action = "{{ route('income.store') }}";
                incomeForm.querySelector('input[name="_method"]').value = 'POST';
                incomeForm.reset(); // Clear form fields
                document.getElementById('income_id').value = ''; // Clear ID for create
                openModal(incomeModal);
            });
        }

        // Edit Income Button (Delegated event listener)
        document.getElementById('income-list-container').addEventListener('click', function (event) {
            if (event.target.classList.contains('edit-income-btn')) {
                const incomeId = event.target.dataset.id;
                modalTitle.textContent = 'Edit Income';
                incomeForm.action = `/income/${incomeId}`; // Laravel route will handle PUT
                incomeForm.querySelector('input[name="_method"]').value = 'PUT';
                ...
                fetch(`/income/${incomeId}/edit`)
                    ...
                    .then(data => {
                        ...
                        document.getElementById('income_id').value = data.id; // Set ID for update
                        openModal(incomeModal);
                    })
                    .catch(error => console.error('Error fetching income for edit:', error));
            }

            // View Income Button (Delegated event listener)
            if (event.target.classList.contains('view-income-btn')) {
                const incomeId = event.target.dataset.id;
                fetch(`/income/${incomeId}`)
                    .then(response => response.text()) // Fetch HTML partial
                    .then(html => {
                        document.getElementById('viewIncomeContent').innerHTML = html;
                        openModal(viewIncomeModal);
                    })
                    .catch(error => console.error('Error fetching income for view:', error));
            }

            // Delete Income Button (Delegated event listener)
