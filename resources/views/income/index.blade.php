...
<div class="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
    <div class="bg-white shadow-lg rounded-lg p-3 sm:p-6 mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 space-y-2 sm:space-y-0">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Income Dashboard</h1>
            @can('create income')
                <button class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out w-full sm:w-auto" id="createIncomeBtn">
                    Add New Income
                </button>
            @endcan
        </div>
        ...
        <form action="{{ route('income.index') }}" method="GET" class="mb-4 sm:mb-6" id="income-search-form">
    ...
    <div id="incomeFormModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center">
        <div class="relative mx-auto top-10 sm:top-20 p-2 sm:p-5 border w-11/12 sm:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-2 sm:pb-3">
                <h3 class="text-xl sm:text-2xl leading-6 font-medium text-gray-900" id="incomeFormModalTitle"></h3>
                <button class="text-gray-400 hover:text-gray-600 close-modal mt-2 sm:mt-0" data-modal-id="incomeFormModal">
            ...
            <div class="mt-2 px-2 sm:px-7 py-2 sm:py-3" id="incomeFormModalContent">
            </div>
        </div>
    </div>

    <div id="viewIncomeModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center">
        <div class="relative mx-auto top-10 sm:top-20 p-2 sm:p-5 border w-11/12 sm:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-2 sm:pb-3">
                <h3 class="text-xl sm:text-2xl leading-6 font-medium text-gray-900">Income Details</h3>
                <button class="text-gray-400 hover:text-gray-600 close-modal mt-2 sm:mt-0" data-modal-id="viewIncomeModal">
                    <svg class="h-5 w-5 sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
            <div class="mt-2 px-2 sm:px-7 py-2 sm:py-3" id="viewIncomeContent">
            </div>
        </div>
    </div>
