<div class="overflow-x-auto rounded-lg shadow-md">
    <table class="min-w-full bg-white">
        ...
        <tbody class="text-gray-600 text-sm font-light">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="py-3 px-6 text-center">
                        <div class="flex item-center justify-center space-x-2">
                            @can('view income')
                                <button class="view-income-btn w-8 h-8 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 flex items-center justify-center transition duration-300" data-id="{{ $income->id }}" title="View">
                            ...
                            @endcan
                            @can('update income')
                                <button class="edit-income-btn w-8 h-8 rounded-full bg-yellow-100 text-yellow-600 hover:bg-yellow-200 flex items-center justify-center transition duration-300" data-id="{{ $income->id }}" title="Edit">
                            ...
                            @can('delete income')
                                <form action="{{ route('income.destroy', $income->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="delete-income-btn w-8 h-8 rounded-full bg-red-100 text-red-600 hover:bg-red-200 flex items-center justify-center transition duration-300" title="Delete">
            ...
            @empty
                <tr>
                    <td colspan="5" class="py-4 px-6 text-center text-gray-500">No income records found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
